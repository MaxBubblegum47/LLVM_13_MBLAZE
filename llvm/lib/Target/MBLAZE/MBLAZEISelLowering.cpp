//===- MBLAZEISelLowering.cpp - MBLAZE DAG Lowering Impl --------------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the MBLAZETargetLowering class.
//
//===----------------------------------------------------------------------===//
...
static MBLAZECC::CondCode ISDCCtoMBLAZECC(ISD::CondCode isdCC) {
  switch (isdCC) {
  case ISD::SETUEQ:
    return MBLAZECC::EQ;
  case ISD::SETUGT:
...
MBLAZETargetLowering::MBLAZETargetLowering(const TargetMachine &TM,
...
const char *MBLAZETargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  case MBLAZEISD::BL:
    return "MBLAZEISD::BL";
  case MBLAZEISD::CMOV:
    return "MBLAZEISD::CMOV";
  case MBLAZEISD::CMP:
    return "MBLAZEISD::CMP";
  case MBLAZEISD::BRcc:
...
SDValue MBLAZETargetLowering::LowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const {
...
SDValue MBLAZETargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
...
SDValue MBLAZETargetLowering::LowerJumpTable(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(MBLAZEISD::GAWRAPPER, SDLoc(N), MVT::i32, GA);
}
...
/// MBLAZE call implementation
SDValue MBLAZETargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_MBLAZE);
  ...
  Chain = DAG.getNode(IsDirect ? MBLAZEISD::BL : MBLAZEISD::JL, dl, NodeTys, Ops);
  Glue = Chain.getValue(1);
...
/// MBLAZE formal arguments implementation
SDValue MBLAZETargetLowering::LowerFormalArguments(
...
SDValue MBLAZETargetLowering::LowerCallArguments(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
...
bool MBLAZETargetLowering::CanLowerReturn(
    CallingConv::ID CallConv, MachineFunction &MF, bool IsVarArg,
    const SmallVectorImpl<ISD::OutputArg> &Outs, LLVMContext &Context) const {
  SmallVector<CCValAssign, 16> RVLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, RVLocs, Context);
  if (!CCInfo.CheckReturn(Outs, RetCC_MBLAZE))
    return false;
...
SDValue
MBLAZETargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool IsVarArg,
                               const SmallVectorImpl<ISD::OutputArg> &Outs,
                               const SmallVectorImpl<SDValue> &OutVals,
                               const SDLoc &dl, SelectionDAG &DAG) const {
  auto *AFI = DAG.getMachineFunction().getInfo<MBLAZEFunctionInfo>();
  ...
  return DAG.getNode(MBLAZEISD::RET, dl, MVT::Other, RetOps);
}
...
SDValue MBLAZETargetLowering::PerformDAGCombine(SDNode *N,
...
bool MBLAZETargetLowering::isLegalAddressingMode(const DataLayout &DL,
...
bool MBLAZETargetLowering::mayBeEmittedAsTailCall(const CallInst *CI) const {
  return false;
}
...
SDValue MBLAZETargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  return DAG.getNode(MBLAZEISD::GAWRAPPER, dl, MVT::i32, GA);
}

static SDValue LowerVASTART(SDValue Op, SelectionDAG &DAG) {
  MachineFunction &MF = DAG.getMachineFunction();
  auto *FuncInfo = MF.getInfo<MBLAZEFunctionInfo>();
...
SDValue MBLAZETargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
