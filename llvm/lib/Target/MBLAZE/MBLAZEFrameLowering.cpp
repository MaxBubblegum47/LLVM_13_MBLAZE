...
//===----------------------------------------------------------------------===//
//
// This file contains the MBLAZE implementation of the TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
static cl::opt<bool>
    UseSaveRestoreFunclet("MBLAZE-save-restore-funclet", cl::Hidden,
                          cl::desc("Use MBLAZE callee save/restore functions"),
                          cl::init(true));
...
static void generateStackAdjustment(MachineBasicBlock &MBB,
                                    ...
                                    const MBLAZEInstrInfo &TII, DebugLoc dl,
  ...
  if (isUInt<6>(AbsAmount))
    AdjOp = Positive ? MBLAZE::ADD_rru6 : MBLAZE::SUB_rru6;
  else if (isInt<12>(AbsAmount))
    AdjOp = Positive ? MBLAZE::ADD_rrs12 : MBLAZE::SUB_rrs12;
  else
    AdjOp = Positive ? MBLAZE::ADD_rrlimm : MBLAZE::SUB_rrlimm;
...
static unsigned determineLastCalleeSave(ArrayRef<CalleeSavedInfo> CSI) {
  unsigned Last = 0;
  for (auto Reg : CSI) {
    assert(Reg.getReg() >= MBLAZE::R13 && Reg.getReg() <= MBLAZE::R25 &&
...
void MBLAZEFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                            BitVector &SavedRegs,
                                            RegScavenger *RS) const {
  LLVM_DEBUG(dbgs() << "Determine Callee Saves: " << MF.getName() << "\n");
  TargetFrameLowering::determineCalleeSaves(MF, SavedRegs, RS);
  SavedRegs.set(MBLAZE::BLINK);
}

void MBLAZEFrameLowering::adjustStackToMatchRecords(
  ...
  generateStackAdjustment(MBB, MBBI, *ST.getInstrInfo(), DebugLoc(),
                          ScalarAlloc, MBLAZE::SP);
}

/// Insert prolog code into the function.
/// For MBLAZE, this inserts a call to a function that puts required callee saved
...
void MBLAZEFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  if (MF.getFunction().isVarArg()) {
    ...
    unsigned Opc = MBLAZE::SUB_rrlimm;
    if (isUInt<6>(VarArgsBytes))
      Opc = MBLAZE::SUB_rru6;
    else if (isInt<12>(VarArgsBytes))
      Opc = MBLAZE::SUB_rrs12;
    BuildMI(MBB, MBBI, dl, TII->get(Opc), MBLAZE::SP)
        .addReg(MBLAZE::SP)
        .addImm(VarArgsBytes);
  }
  if (hasFP(MF)) {
    LLVM_DEBUG(dbgs() << "Saving FP\n");
    BuildMI(MBB, MBBI, dl, TII->get(MBLAZE::ST_AW_rs9))
        .addReg(MBLAZE::SP, RegState::Define)
        .addReg(MBLAZE::FP)
        .addReg(MBLAZE::SP)
        .addImm(-4);
    AlreadyAdjusted += 4;
  }
  if (UseSaveRestoreFunclet && Last > MBLAZE::R14) {
    LLVM_DEBUG(dbgs() << "Creating store funclet.\n");
    // BL to __save_r13_to_<TRI->getRegAsmName()>
    StackSlotsUsedByFunclet = Last - MBLAZE::R12;
    BuildMI(MBB, MBBI, dl, TII->get(MBLAZE::PUSH_S_BLINK));
    BuildMI(MBB, MBBI, dl, TII->get(MBLAZE::SUB_rru6))
        .addReg(MBLAZE::SP)
        .addReg(MBLAZE::SP)
        .addImm(4 * StackSlotsUsedByFunclet);
    BuildMI(MBB, MBBI, dl, TII->get(MBLAZE::BL))
        .addExternalSymbol(store_funclet_name[Last - MBLAZE::R15])
        .addReg(MBLAZE::BLINK, RegState::Implicit | RegState::Kill);
  ...
  generateStackAdjustment(MBB, MBBI, *ST.getInstrInfo(), dl,
                          -(MFI.getStackSize() - AlreadyAdjusted), MBLAZE::SP);

  if (hasFP(MF)) {
    LLVM_DEBUG(dbgs() << "Setting FP from SP.\n");
    BuildMI(MBB, MBBI, dl,
            TII->get(isUInt<6>(MFI.getStackSize()) ? MBLAZE::ADD_rru6
                                                   : MBLAZE::ADD_rrlimm),
            MBLAZE::FP)
        .addReg(MBLAZE::SP)
  ...
  if (hasFP(MF)) {
    CFIIndex = MF.addFrameInst(MCCFIInstruction::createOffset(
        nullptr, MRI->getDwarfRegNum(MBLAZE::FP, true), CurOffset));
    BuildMI(MBB, MBBI, dl, TII->get(TargetOpcode::CFI_INSTRUCTION))
        .addCFIIndex(CFIIndex)
        .setMIFlags(MachineInstr::FrameSetup);
    CurOffset -= 4;
  ...
  if (MFI.hasCalls()) {
    CFIIndex = MF.addFrameInst(MCCFIInstruction::createOffset(
        nullptr, MRI->getDwarfRegNum(MBLAZE::BLINK, true), CurOffset));
    BuildMI(MBB, MBBI, dl, TII->get(TargetOpcode::CFI_INSTRUCTION))
        .addCFIIndex(CFIIndex)
        .setMIFlags(MachineInstr::FrameSetup);
  }
  ...
  for (const auto &Entry : CSI) {
    ...
    // Skip BLINK and FP.
    if ((hasFP(MF) && Reg == MBLAZE::FP) || (MFI.hasCalls() && Reg == MBLAZE::BLINK))
      continue;
    CFIIndex = MF.addFrameInst(MCCFIInstruction::createOffset(
...
/// For MBLAZE, this inserts a call to a function that restores callee saved
/// registers onto the stack, when enough callee saved registers are required.
void MBLAZEFrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  if (hasFP(MF)) {
    ...
    if (isUInt<6>(StackSize))
      Opc = MBLAZE::SUB_rru6;
    BuildMI(MBB, MBBI, DebugLoc(), TII->get(Opc), MBLAZE::SP)
        .addReg(MBLAZE::FP)
        .addImm(StackSize);
  ...
  if (unsigned MoveAmount = StackSize - AmountAboveFunclet) {
    unsigned Opc = MBLAZE::ADD_rrlimm;
    if (isUInt<6>(MoveAmount))
      Opc = MBLAZE::ADD_rru6;
    else if (isInt<12>(MoveAmount))
      Opc = MBLAZE::ADD_rrs12;
    BuildMI(MBB, MBBI, MBB.findDebugLoc(MBBI), TII->get(Opc), MBLAZE::SP)
        .addReg(MBLAZE::SP)
  ...
  if (StackSlotsUsedByFunclet) {
    ...
    BuildMI(MBB, MBBI, MBB.findDebugLoc(MBBI), TII->get(MBLAZE::BL))
        .addExternalSymbol(load_funclet_name[Last - MBLAZE::R15])
        .addReg(MBLAZE::BLINK, RegState::Implicit | RegState::Kill);
    unsigned Opc = MBLAZE::ADD_rrlimm;
  ...
  if (SavedBlink) {
    BuildMI(MBB, MBBI, MBB.findDebugLoc(MBBI), TII->get(MBLAZE::POP_S_BLINK));
  }
  // Now, pop fp if necessary.
  if (hasFP(MF)) {
    BuildMI(MBB, MBBI, MBB.findDebugLoc(MBBI), TII->get(MBLAZE::LD_AB_rs9))
        .addReg(MBLAZE::FP, RegState::Define)
        .addReg(MBLAZE::SP, RegState::Define)
        .addReg(MBLAZE::SP)
        .addImm(4);
  }
...
bool MBLAZEFrameLowering::assignCalleeSavedSpillSlots(
  ...
  if (MFI.hasCalls() || (UseSaveRestoreFunclet && Last > MBLAZE::R14)) {
  ...
  for (unsigned Which = Last; Which > MBLAZE::R12; Which--) {
...
bool MBLAZEFrameLowering::spillCalleeSavedRegisters(
...
bool MBLAZEFrameLowering::restoreCalleeSavedRegisters(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MI,
...
void MBLAZEFrameLowering::processFunctionBeforeFrameFinalized(
  ...
  const TargetRegisterClass *RC = &MBLAZE::GPR32RegClass;
...
static void emitRegUpdate(MachineBasicBlock &MBB,
  ...
  if (isUInt<6>(NumBytes))
    Opc = IsAdd ? MBLAZE::ADD_rru6 : MBLAZE::SUB_rru6;
  else if (isInt<12>(NumBytes))
    Opc = IsAdd ? MBLAZE::ADD_rrs12 : MBLAZE::SUB_rrs12;
  else
    Opc = IsAdd ? MBLAZE::ADD_rrlimm : MBLAZE::SUB_rrlimm;
...
MachineBasicBlock::iterator MBLAZEFrameLowering::eliminateCallFramePseudoInstr(
  ...
  auto *AFI = MF.getInfo<MBLAZEFunctionInfo>();
  if (!hasFP(MF)) {
    if (Amt > AFI->MaxCallStackReq && Old.getOpcode() == MBLAZE::ADJCALLSTACKDOWN)
      AFI->MaxCallStackReq = Amt;
  } else {
    if (Amt != 0) {
      assert((Old.getOpcode() == MBLAZE::ADJCALLSTACKDOWN ||
              Old.getOpcode() == MBLAZE::ADJCALLSTACKUP) &&
             "Unknown Frame Pseudo.");
      bool IsAdd = (Old.getOpcode() == MBLAZE::ADJCALLSTACKUP);
      emitRegUpdate(MBB, I, dl, MBLAZE::SP, Amt, IsAdd, TII);
    }
  }
  return MBB.erase(I);
}
